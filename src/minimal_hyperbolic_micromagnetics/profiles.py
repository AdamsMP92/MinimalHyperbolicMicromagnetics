...
def profile_derivatives(
    ...
    """Return analytic first and second derivatives of all energy profiles.

    Derivatives are taken with respect to the dimensionless vortex coordinate
    ``nu``.  The local-profile derivatives follow by differentiating their
    one-dimensional integrands.  Magnetostatic derivatives are obtained by
    differentiating the Legendre moments before forming their quadratic sum.
    """
    nu_arr, scalar_input, nu_flat = _as_array(nu)

    local_start = time.perf_counter()
    if verbose:
        print("[profiles] analytic local-profile derivatives: start", flush=True)
    x, w = _gauss_legendre_interval(n_quad, 0.0, 1.0)
    sqrt_weight = np.sqrt(np.maximum(0.0, 1.0 - x * x))
    volume_weight = x * sqrt_weight

    derivatives = {
        name: np.empty_like(nu_flat)
        for name in (
            "g_ex_d1",
            "g_ex_d2",
            "g_u_x_d1",
    ...
    for start in range(0, len(nu_flat), chunk_size):
        ...
        guz_d1_integrand = -2.0 * x[None, :] * sech2 * tanh_psi
        guz_d2_integrand = (
            2.0
            * x[None, :] ** 2
            * sech2
        ...
        gzz_d1_integrand = -x[None, :] * sech_psi * tanh_psi
        gzz_d2_integrand = (
            x[None, :] ** 2
            * sech_psi
            * (tanh_psi**2 - sech2)
        ...
        guz_d1 = 3.0 * np.sum(
            w[None, :] * volume_weight[None, :] * guz_d1_integrand,
            axis=1,
        )
        guz_d2 = 3.0 * np.sum(
            w[None, :] * volume_weight[None, :] * guz_d2_integrand,
            axis=1,
        )
        gzz_d1 = 3.0 * np.sum(
            w[None, :] * volume_weight[None, :] * gzz_d1_integrand,
            axis=1,
        )
        gzz_d2 = 3.0 * np.sum(
            w[None, :] * volume_weight[None, :] * gzz_d2_integrand,
            axis=1,
        )

        gex_d1_integrand = sqrt_weight[None, :] * (
            2.0 * nu_chunk * x[None, :] * sech2
            - 2.0
            * nu_chunk**2
            * x[None, :] ** 2
            * sech2
        ...
        gex_d2_integrand = sqrt_weight[None, :] * (
            2.0 * x[None, :] * sech2
            - 8.0
            * nu_chunk
            * x[None, :] ** 2
        ...
        derivatives["g_ex_d1"][start:stop] = 3.0 * np.sum(
            w[None, :] * gex_d1_integrand,
            axis=1,
        )
        derivatives["g_ex_d2"][start:stop] = 3.0 * np.sum(
            w[None, :] * gex_d2_integrand,
            axis=1,
        )
    ...
    if verbose:
        ...
        print(
            "[profiles] analytic local-profile derivatives: "
            f"done in {elapsed:.3f} s",
            flush=True,
        )

    demag_start = time.perf_counter()
    if verbose:
        print("[profiles] analytic magnetostatic derivatives: start", flush=True)
    if l_max < 1:
        raise ValueError("l_max must be at least 1")
    if l_max % 2 == 0:
        l_max -= 1
    ...
    x_demag, w_demag = _gauss_legendre_interval(n_mu, 0.0, 1.0)
    mu_positive = np.sqrt(np.maximum(0.0, 1.0 - x_demag * x_demag))
    legendre_values = np.polynomial.legendre.legvander(mu_positive, l_max)
    p_odd = legendre_values[:, np.arange(1, l_max + 1, 2)]
    gdem_d1 = np.empty_like(nu_flat)
    gdem_d2 = np.empty_like(nu_flat)

    for index, nu_i in enumerate(nu_flat):
        psi = nu_i * x_demag
        sech_psi = _sech(psi)
        tanh_psi = np.tanh(psi)
        moments = (w_demag * x_demag * sech_psi) @ p_odd
        moments_d1 = (
            -w_demag * x_demag**2 * sech_psi * tanh_psi
        ) @ p_odd
        moments_d2 = (
            w_demag
            * x_demag**3
            * sech_psi
            * (tanh_psi**2 - sech_psi**2)
        ) @ p_odd
        gdem_d1[index] = -3.0 * np.sum(moments * moments_d1)
        gdem_d2[index] = -3.0 * np.sum(
            moments_d1**2 + moments * moments_d2
        )

    derivatives["g_dem_d1"] = gdem_d1
    derivatives["g_dem_d2"] = gdem_d2

    if verbose:
        elapsed = time.perf_counter() - demag_start
        print(
            "[profiles] analytic magnetostatic derivatives: "
            f"done in {elapsed:.3f} s",
            flush=True,
        )
    ...
    if np.any(zero_mask):
        ...
        for name, value in exact_at_zero.items():
            derivatives[name][zero_mask] = value

    if scalar_input:
        return {name: float(values[0]) for name, values in derivatives.items()}
    return {
        name: values.reshape(nu_arr.shape)
...
def all_profiles(
    ...
    """Return all optimized profile functions with optional stage timings."""
    _, _, nu_flat = _as_array(nu)
    total_start = time.perf_counter()
    if verbose:
        effective_l_max = l_max if l_max % 2 else l_max - 1
        effective_n_mu = (
            n_mu if n_mu is not None else max(360, 4 * (effective_l_max + 1))
        )
        print(
            "[profiles] tabulation: start "
            f"(n_nu={len(nu_flat)}, n_quad={n_quad}, "
            f"l_max={effective_l_max}, n_mu={effective_n_mu})",
            flush=True,
        )

    stage_start = time.perf_counter()
    if verbose:
        print("[profiles] local energy-profile integrals: start", flush=True)
    ...
    if verbose:
        ...
        print(
            "[profiles] local energy-profile integrals: "
            f"done in {elapsed:.3f} s",
            flush=True,
        )
    ...
    stage_start = time.perf_counter()
    if verbose:
        print("[profiles] magnetostatic Legendre-moment sum: start", flush=True)
    gh = g_hd_positive(nu, l_max=l_max, n_mu=n_mu)
    if verbose:
        elapsed = time.perf_counter() - stage_start
        print(
            "[profiles] magnetostatic Legendre-moment sum: "
            f"done in {elapsed:.3f} s",
            flush=True,
        )
    gs_b = gh - 0.5
    gdem = -gs_b
    derivatives = profile_derivatives(
        nu,
        n_quad=n_quad,
        chunk_size=chunk_size,
        l_max=l_max,
    ...
    if verbose:
        ...
        print(f"[profiles] tabulation: done in {elapsed:.3f} s", flush=True)
...
def compute_and_store_profiles(
    ...
    *,
    ...
    profiles = compute_profiles(settings, nu=nu, verbose=verbose)
    ...
    if verbose:
        ...
        print(
            f"[profiles] CSV output: done in {elapsed:.3f} s ({output_csv})",
            flush=True,
        )
    ...
    if make_plot or output_png is not None:
        ...
        if verbose:
            elapsed = time.perf_counter() - plot_start
            print(
                f"[profiles] profile plot: done in {elapsed:.3f} s ({output_png})",
                flush=True,
            )
